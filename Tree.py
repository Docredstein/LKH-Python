...
class Node:
    ...
    def __copy__(self):
        return Node(
            self.id, self.left, self.right, self.parent, self.key, self.keyid, self.user
        )
    ...
    def __repr__(self, prefix="") -> str:
        return (
            f"{prefix}|Node {self.id} {self.keyid} [{Fore.YELLOW}{self.key.hex()}{Fore.RESET}] : {self.user.userID if self.user is not None else "None"}"
            + ("" if self.left is None else f"\n{self.left.__repr__(prefix+"\t")}")
            + ("" if self.right is None else f"\n{self.right.__repr__(prefix+"\t")}")
        )
...
class LKH:
    ...
    def sendKeyToChildren(self, current: Node):
        assert current.left is not None
        assert current.right is not None
        packetData = KeyUpdatePacket(
            newKey=current.key,
            newKeyid=current.keyid,
            isSessionKey=(current == self.root),
            deleteNewKey=False,
        )
        if current.left.key != b"":
            packet = self.encrypt(
                key=current.left.key,
                data=packetData.toBytes(),
                aad=current.left.keyid.to_bytes(8),
            )

            if self.debug:
                print(
                    f"Sending keys {current.keyid} to node {current.left.id}: {packet.hex()}"
                )
            self.sendGroup(packet)
        elif self.debug:
            print(
                f"Didn't send new keys to {current.left.id} because no keys have been assigned"
            )
        ...
        if current.right.key != b"":
            packet = self.encrypt(
                ...
                data=packetData.toBytes(),
                aad=current.right.keyid.to_bytes(8),
            )

            if self.debug:
                print(
                    f"Sending keys {current.keyid} to node {current.right.id}: {packet.hex()}"
                )
            self.sendGroup(packet)
        elif self.debug:
            print(
                f"Didn't send new keys to {current.right.id} because no keys have been assigned"
            )

    def sendKeyUsingUnicast(self, node: Node, path: dict[int, bytes]):
    ...
    def updateKey(
        ...
        alreadyUpdated: set[int] | None = None,
    ):
        """
        ...
        for i in nodesToDelete:
            ...
            updatePacket = KeyUpdatePacket(
                newKey=i.key, newKeyid=i.keyid, isSessionKey=False, deleteNewKey=True
            ).toBytes()
            self.encrypt(
                key=self.root.key, data=updatePacket, aad=self.root.keyid.to_bytes(8)
            )
    ...
    def splitNode(
        self, nodeToSplit: Node, userToAdd: User, shouldUpdateKey: bool = True
    ):
        """
    ...
    def addUserGroup(self, users: Sequence[User]) -> None:
        ...
        for user in users:
            ...
            if len(self.users) <= 0:
                ...
                self.depth[0] = set([self.root.keyid])
                self.nodes[self.root.keyid] = self.root
                # travelByLayer[0] = set([user.userID])
            else:
                targetDepth = min([i for i in self.depth if len(self.depth[i]) > 0])
                targetParentNode = self.nodes[next(iter(self.depth[targetDepth]))]
                self.splitNode(targetParentNode, user, False)

        travelByLayer: dict[int, set[Node]] = {}
        for user in users:
            currentNode = self.users[user.userID]
            if currentNode.depth in travelByLayer:
                travelByLayer[currentNode.depth].add(currentNode)
            else:
                travelByLayer[currentNode.depth] = set([currentNode])

        self.updateKeyByLayer(travelByLayer)
    ...
    def removeUser(self, user: User):
        if user.userID not in self.users:
            raise Exception(
                f"User {user.userID} not found in tree, available users {self.users}"
            )
        if self.debug:
            print(
...
def draw_tree_matplotlib(
    ...
    def draw_node(node: Node, x, y, dx, ax):
        ...
        # Dessiner le noeud
        if node.keyid in specialKeys:
            nodeColor = "lightcoral"
        elif node.user is not None:
